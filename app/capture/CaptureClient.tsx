...
import { CapturePromptChips } from '@/components/capture/CapturePromptChips'
import { DailyRitualStrip } from '@/components/capture/DailyRitualStrip'
import { getCapturePhase, phaseHeadline, promptsForPhase } from '@/lib/capture/daily-prompts'
...
export default function CaptureClient() {
  ...
  const listenOnceRef = useRef(false)
  const starterAppliedRef = useRef(false)

  const phase = getCapturePhase()
  const { title: phaseTitle, subtitle: phaseSubtitle } = phaseHeadline(phase)
  const promptChips = promptsForPhase(phase).slice(0, 5)
  ...
  useEffect(() => {
    ...
    if (starter && !starterAppliedRef.current && pageState === 'idle') {
  ...
  useEffect(() => {
    if (
      searchParams.get('listen') === '1'
      && apiKey
      && isSupported
      && !listenOnceRef.current
      && pageState === 'idle'
    ) {
      listenOnceRef.current = true
      reset()
      start()
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [searchParams, apiKey, isSupported, pageState])
  ...
  const loadTodayRitual = useCallback(async () => {
    try {
      const entry = await dailyStorage.getByDate(todayISO())
      setHasMorning(!!entry?.morning)
      setHasEvening(!!entry?.evening)
    } catch {
      // Non-fatal
    }
  }, [])
  ...
  const handleTextareaKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    if ((e.metaKey || e.ctrlKey) && e.key === 'Enter' && activeTranscript.trim() && pageState === 'idle') {
      e.preventDefault()
      process()
    }
  }
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-5 pb-28 sm:pb-5">
      {/* ── Header ─────────────────────────────────────────────────────────── */}
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-sm text-[#6b6b88]">{phaseSubtitle}</p>
        </div>
      ...
      <div className="rounded-xl border border-[#1e1e2a] bg-[#111116] px-3 py-3">
        <DailyRitualStrip
        ...
        <div className="rounded-xl border border-[#1e1e2a] bg-[#111116] p-4">
          ...
          <div className="relative">
            <textarea
              ...
              placeholder="Speak or type — ⌘↵ to process…"
              rows={4}
          ...
          <div className="mt-3 hidden justify-end sm:flex">
            ...
            >
              {pageState === 'processing' ? (
                ...
                <><Send className="h-3.5 w-3.5" /> Process <span className="text-indigo-200/60 text-[10px]">⌘↵</span></>
              )}
            </button>
          </div>
      ...
      {(pageState === 'idle' || pageState === 'processing') && (
        <div className="fixed inset-x-0 bottom-0 z-50 border-t border-[#1e1e2a] bg-[#0a0a0c]/95 px-4 py-3 backdrop-blur sm:hidden">
          <div className="mx-auto flex max-w-2xl items-center gap-3">
            <button
              ...
              className={cn(
                ...
                voiceState === 'listening'
                  ? 'animate-pulse border-indigo-400 bg-indigo-500/20 text-indigo-300'
                  : 'border-[#1e1e2a] bg-[#111116] text-[#8080a0]'
              )}
            >
              {voiceState === 'listening' ? <MicOff className="h-6 w-6" /> : <Mic className="h-6 w-6" />}
            </button>
            <button
              type="button"
              onClick={process}
              disabled={!activeTranscript.trim() || pageState === 'processing'}
              className={cn(
                'flex flex-1 items-center justify-center gap-2 rounded-xl py-3.5 text-sm font-medium transition-all',
                activeTranscript.trim() && pageState !== 'processing'
            ...
            >
              {pageState === 'processing' ? (
                ...
                <><Send className="h-4 w-4" /> Process</>
              )}
            </button>
          </div>
        </div>
